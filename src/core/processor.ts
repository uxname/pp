...
import type { FileWithScore } from '../types'; // <--- import type
...
export function generateOutputContent(
    ...
    for (const file of sortedFiles) {
        // Используем file.relativePath, так как мы его добавили в сканере
        const relativePath = file.relativePath || relative(rootPath, file.path);

        try {
            if (file.size > maxFileSizeBytes) {
                skippedFiles.push(`${relativePath} (file too large: ${file.size} bytes)`);
                continue;
            }
            ...
            const msg = error instanceof Error ? error.message : String(error); // <--- fix error message
