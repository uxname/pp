...
import type { HistoryItem } from '../types'; // <--- import type
...
function ensureHistoryFileExists(): void {
    ...
    if (!existsSync(HISTORY_FILE)) writeFileSync(HISTORY_FILE, JSON.stringify([], null, 2), 'utf-8');
}
...
export async function showHistoryMenu(): Promise<string[] | null> {
    ...
    // Fix TS2532: Проверка на корректность индекса
